...
export default defineEventHandler((event) => {
    ...
    if (index > -1) {
        ...
        if (!tokenHeader) {
            ...
            return { detail: 'The Authorization header is not defined, either it is empty, or it does not have a Bearer <your_token> template' }
        }

        const token = tokenHeader.match(/^Bearer\s+(.+)$/)?.[1];
        if (!token) {
            setResponseStatus(event, 401, 'Unauthorized')
            return { detail: 'Header template Authorization does not match template: Bearer <your_token>' }
        }
